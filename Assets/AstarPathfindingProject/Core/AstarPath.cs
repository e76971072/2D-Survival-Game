...
public class AstarPath : VersionedMonoBehaviour
{
    ...
    private void LogPathResults(Path path)
    {
        ...
        {
            var debug = path.DebugString(logPathResults);
    ...
    internal void VerifyIntegrity()
    {
        ...
        if (data == null) throw new System.NullReferenceException("data is null... A* not set up correctly?");
    ...
    public NNInfo GetNearest(Vector3 position, NNConstraint constraint, GraphNode hint)
    {
        ...
        if (graphs != null)
            for (var i = 0; i < graphs.Length; i++)
            {
                ...
                if (fullGetNearestSearch) // Slower nearest node search
        ...
        if (!constraint.Suitable(nearestNode.node) || constraint.constrainDistance &&
            (nearestNode.clampedPosition - position).sqrMagnitude > maxNearestNodeDistanceSqr) return new NNInfo();
