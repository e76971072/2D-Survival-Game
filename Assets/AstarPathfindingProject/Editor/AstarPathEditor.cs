...
namespace Pathfinding
{
    ...
    public class AstarPathEditor : Editor
    {
        ...
        private static Dictionary<string, CustomGraphEditorAttribute> graphEditorTypes =
        ...
        private System.Type[] graphTypes => script.data.graphTypes;
        ...
        private static FadeArea settingsArea;
        private static FadeArea colorSettingsArea;
        private static FadeArea editorSettingsArea;
        private static FadeArea aboutArea;
        private static FadeArea optimizationSettingsArea;
        private static FadeArea serializationSettingsArea;
        private static FadeArea tagsArea;
        private static FadeArea graphsArea;
        private static FadeArea addGraphsArea;
        private static FadeArea alwaysVisibleArea;
        ...
        private void DrawSerializationSettings()
        {
            ...
            if (serializationSettingsArea.BeginFade())
            {
                ...
                {
                    var serializationSettings = new Serialization.SerializeSettings();
                ...
                if (GUILayout.Button("Save to file"))
                {
                    ...
                    if (path != "")
                    {
                        var serializationSettings = Serialization.SerializeSettings.Settings;
                        ...
                        Serialization.AstarSerializer.SaveToFile(path, bytes);
                ...
                if (GUILayout.Button("Load from file"))
                {
                    ...
                    if (path != "")
                        try
                        {
                            var bytes = Serialization.AstarSerializer.LoadFromFile(path);
        ...
        private void CheckGraphEditors(bool forceRebuild = false)
        {
                ...
                script.graphs.Length != graphEditors.Length)
            {
                ...
                for (var i = 0; i < script.graphs.Length; i++)
                {
                    ...
                    if (graph.guid == new Util.Guid()) graph.guid = Util.Guid.NewGuid();
            ...
            else
            {
                for (var i = 0; i < script.graphs.Length; i++)
                {
                    ...
                    if (script.graphs[i].guid == new Util.Guid()) script.graphs[i].guid = Util.Guid.NewGuid();
        ...
        {
            var settings = Serialization.SerializeSettings.Settings;

            settings.editorSettings = true;
            return SerializeGraphs(settings, out checksum);
        }

        public byte[] SerializeGraphs(Serialization.SerializeSettings settings, out uint checksum)
        {
            byte[] bytes = null;
            uint tmpChecksum = 0;
            ...
            for (var i = 0; i < graphEditors.Length; i++)
            {
                ...
                output.Length = 0;
                Serialization.TinyJsonSerializer.Serialize(graphEditors[i], output);
        ...
        private void DeserializeGraphs(byte[] bytes)
        {
            try
            {
                ...
                for (var i = 0; i < graphEditors.Length; i++)
                {
                    ...
                    if (data != null)
                        Serialization.TinyJsonDeserializer.Deserialize(data, graphEditors[i].GetType(),
                            graphEditors[i]);
                }
            }
            catch (System.Exception e)
