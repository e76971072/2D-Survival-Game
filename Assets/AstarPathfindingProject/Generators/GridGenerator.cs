...
namespace Pathfinding
{
    ...
    public class GridGraph : NavGraph, IUpdatableGraph, ITransformedGraph
    {
        ...
        public GraphTransform CalculateTransform()
        {
            ...
            var m = Matrix4x4.TRS(
                        boundsMatrix.MultiplyPoint3x4(-new Vector3(newWidth * newNodeSize, 0, newDepth * newNodeSize) *
                                                      0.5F), Quaternion.Euler(rotation),
                        new Vector3(newNodeSize * aspectRatio, 1, newNodeSize)) * isometricMatrix;
        ...
        protected override IEnumerable<Progress> ScanInternal()
        {
            ...
            for (var z = 0; z < depth; z++)
            {
                ...
                for (var x = 0; x < width; x++) // Updates the position of the node
            ...
            for (var z = 0; z < depth; z++)
            {
                ...
                for (var x = 0; x < width; x++) // Recalculate connections to other nodes
        ...
        private void ErodeNodeWithTags(GraphNode node, int iteration)
        {
            ...
            {
                if (neighbours == NumNeighbours.Six) // Check the 6 hexagonal connections
                    for (var i = 0; i < 6; i++)
                    {
                ...
                else // Check the four axis aligned connections
                    for (var i = 0; i < 4; i++)
                    {
            ...
            if (neighbours == NumNeighbours.Four || neighbours == NumNeighbours.Eight)
            {
                ...
                for (var i = 0; i < 4; i++)
                {
                    ...
                    if ((nx >= 0) & (nz >= 0) & (nx < width) & (nz < depth))
                    {
                        ...
                        if (IsValidConnection(node, other)) // Enable connection i
                            conns |= 1 << i;
                    }
                }
                ...
                if (neighbours == NumNeighbours.Eight)
                {
                    if (cutCorners)
                    {
                        for (var i = 0; i < 4; i++) // If at least one axis aligned connection
                    ...
                    else
                    {
                        for (var i = 0;
                                i < 4;
                                i++) // If exactly 2 axis aligned connections is adjacent to this connection
                ...
                if (!gizmos.Draw(hasher))
                {
                    ...
                    using (var helper = gizmos.GetGizmoHelper(active, hasher))
                    {
                        if (showNodeConnections)
                            for (var i = 0; i < allNodesCount; i++) // Don't bother drawing unwalkable nodes
            ...
            out IntRect physicsRect, out bool willChangeWalkability, out int erosion)
        {
            ...
            if (o.updatePhysics && !o.modifyWalkability) // Add the collision.diameter margin for physics calls
        ...
        void IUpdatableGraph.UpdateArea(GraphUpdateObject o)
        {
            ...
            else if (willChangeWalkability && erosion > 0)
            {
                ...
                for (var z = erosionRect2.ymin; z <= erosionRect2.ymax; z++)
                {
                    ...
                    if (!erosionRect1.Contains(x, z)) //Save the border's walkabilty data (will be reset later)
